...
us_fiscal_lsuw_tmp    = ts(as.matrix(us_fiscal_lsuw_tmp), start = c(1948, 1), frequency = 4)

# Romer, Romer, instrument extrapolated to 2026
############################################################
fiscal_shocks   = read.csv(file = paste0(PATH,"spikes_by_ac.csv"))
fiscal_time     = zoo::index(us_fiscal_lsuw_tmp)
us_fiscal       = cbind(us_fiscal_lsuw_tmp, 0)
...
save(
  us_fiscal, 
  file = "data/us_fiscal.rda"
)


# WARNING!!! The code below does not set the dates automatically
# NBER quarterly recession indicator
############################################################
usrec   = fredr::fredr(series_id = "USRECQ")
usrec   = xts::xts(usrec$value, as.Date(usrec$date))
us_recessions   = ts(
  usrec[as.Date(zoo::index(usrec)) > as.Date("1947-12-31") & as.Date(zoo::index(usrec)) < as.Date("2026-01-01")],
  start = c(1948, 1),
  frequency = 4,
  names = "usrec"
)  
