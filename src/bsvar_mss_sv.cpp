...
Rcpp::List bsvar_mss_sv_cpp (
  ...
  for (int ss=0; ss<SS; ss++) {
    ...
    try {
      aux_hyper_tmp   = sample_hyperparameters_mss( aux_hyper, aux_B, aux_A, VB, prior);
    } catch(...) {
      acceptance_count(1)++;
    }
    ...
    aux_B_tmp         = aux_B;
    try {
      aux_B_tmp       = sample_B_mss(aux_B, aux_A, aux_hyper, aux_sigma, aux_xi, Y, X, prior, VB);
    } catch (...) {
      acceptance_count(2)++;
    }
    aux_B             = aux_B_tmp;
